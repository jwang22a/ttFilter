...
public class ProcessService {
	public String createProcess(String modelid,String UserId){
		//String modelid="LocalAcctAbnormalOpeAudit";
		//更新ACTIVITYS_CONFIG的状态为1，准备工作
		String resultMessage="";
		PrepareedDao.prepareProcess(modelid);
       ...
       //sql = "select * from(select * from "+target_table+" where STATU=0  order by dbms_random.value) where rownum<630000  ";
        ...
        for (int j=0;j<data_list.size();j++){
            ...
            String roleId=udao.getUserRoleId(UserId);
            data_map.put("drafter", roleId);
            String data_json = data_map.toJSONString();
            System.out.println("paramjson====="+data_json);
            LogManager.infoMessage("paramjson+++++++"+data_json);
            List<NameValuePair> params= new ArrayList<NameValuePair>();
            params.add(new BasicNameValuePair("paramjson",data_map.toJSONString()));
            String app_return_str="";
            try {
            	app_return_str = HttpUtil.postUrl(model_url, HTTP.UTF_8,params,"");
			} catch (Exception e) {
				// TODO: handle exception
            	resultMessage="前"+k2+"条工单生成成功,后面工单生成失败,请调整后重新提交失败工单！";
            	PrepareedDao.updateZJJJ();
            ...
            try {
	            ...
	            if((!"瓒呮椂".equals(app_return_str))&&!StringUtils.isEmpty(app_return_str)&&!StringUtils.isEmpty(app_return_str.trim())){
	                ...
	                if("true".equals(rest)){
	                	LogManager.infoMessage("工单生成成功+++++++"+app_return_str);
	                    k2++;
	                    //鏇存柊鐘舵��
				...
				// TODO: handle exception
            	resultMessage="前"+k2+"条工单生成成功,后面工单生成失败,请调整后重新提交失败工单！";
            	LogManager.infoMessage(resultMessage);
            	PrepareedDao.updateZJJJ();
            	return resultMessage;
			}
